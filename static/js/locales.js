      ...
      'bodyPlaceholder': `请输入请求体，使用JSON Schema格式。例如：
{
    ...
    "required": ["name", "age"]
        ...
        'userfileNotice':'删除数据文件夹中所有文件可以将软件恢复到初始状态，数据文件夹中的logs文件夹中有日志文件，你可以查看文件件中最新的两个log文件以获取软件运行日志',
        'openExtfile': '打开扩展文件夹',
      ...
      'bodyPlaceholder': `Enter the body in JSON Schema format. For example:
{
    ...
    "required": ["name", "age"]
        ...
        'userfileNotice':'Deleting all files in the data folder can restore the software to its initial state. The logs folder within the data folder contains log files. You can check the latest two log files in the folder to obtain the software operation logs.',
